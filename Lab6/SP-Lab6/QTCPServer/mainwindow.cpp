...
void MainWindow::readSocket()
{
    ...
    if(fileType=="attachment"){
        ...
        qDebug() << "Current message:" << message; //Shows message in format "SocketId :: Message"
        emit newMessage(message);
        //Сервер отримує шлях до файлу в змінній message
        //
        QString path = message;
        QDateTime date;
        QString result = date.currentDateTime().toString() + " " + path + " Size: " + formatSize(dirSize(path));
        qDebug() << "Result message:" << result;
        //
        //
        QString receiver = result;
        foreach (QTcpSocket* socket,connection_set)
        {
